...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps & ButtonIconProps & LoadingProps>(
  ({ className, variant, effect, size, icon: Icon, iconPlacement, isLoading, asChild = false, children, ...props }, ref) => {
    const Comp = asChild ? Slot : 'button';
    return (
      <Comp className={cn(buttonVariants({ variant, effect, size, className }))} ref={ref} {...props} disabled={isLoading || props.disabled}>
        {(Icon && !isLoading) &&
          iconPlacement === 'left' &&
          (effect === 'expandIcon' ? (
